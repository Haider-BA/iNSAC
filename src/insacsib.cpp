/** @file insacsib.cpp
 * @brief Driver function for steady-state iNS AC solution with immersed boundaries
 */
...
int main(int argc, char* argv[])
{
	...
	Steady_insac_ib ins;
	ins.setup(&m, rho, mu, bcflags, bvalues, gradscheme, pressurescheme, refvel, cfl, tol, maxiter, true, ibfile);
	ins.solve();

	Array2d<double>* u = ins.getVariables();
	Array2d<double>* residuals = ins.getResiduals();

	Structdata2d strd(&m, u, residuals, "insac_ib");
