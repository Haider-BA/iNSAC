...
class Time_insac
{
	...
	Array2d<double>* uold;			///< Values of the 3 unkowns at the previous time step
...
public:
	...
	*/
	void setup(Structmesh2d* mesh, double dens, double visc, vector<int> _bcflags, vector<vector<double>> _bvalues, string gradscheme, string pressure_scheme, double refvel, double CFL, double tolerance, int maxiters, double total_time, bool is_inviscid = false);
...
Time_insac::Time_insac() {
	isalloc = false;
}

void Time_insac::setup(Structmesh2d* mesh, double dens, double visc, vector<int> _bcflags, vector<vector<double>> _bvalues, string gradscheme, string pressure_scheme, double refvel, double CFL, double tolerance, int maxiters, double total_time, bool is_inviscid)
{
	ndim = 2;
	m = mesh;
	nvar = 3;
...
*/
void Time_insac::compute_timesteps()
{
	int i,j, dim;
	...
	gdt = 10.0;
	//cout << "Time_insac: compute_timesteps(): Now computing time steps for next iteration..." << endl;
...
void Time_insac::setInitialConditions()
{
	int i,j,k;
	for(i = 0; i <= m->gimx(); i++)
		for(j = 0; j <= m->gjmx(); j++)
...
/** Make sure the [Time_insac](@ref Time_insac) object has been [setup](@ref setup) and initialized with some [initial condition](@ref setInitialConditions). Both the momentum-magnitude residual and the mass flux are taken as convergence criteria; the tolerance for the mass flux is the square-root of the tolerance for the relative momentum-magnitude residual, which is  [tol](@ref tol).
*/
void Time_insac::solve()
{
	...
	int i,j,k,nk, nt = 0;
	double resnorm, resnorm0, massflux, dtv, telapsed = 0.0;
	cout << "Time_insac: solve(): Beginning the time-march." << endl;
	
	while(telapsed < tt)
	{
		compute_timesteps();

		if(nt % 10 == 0) cout << "\nTime_insac: solve(): Current physical time step = " << gdt << ", time elapsed = " << telapsed << endl;
		...
		for(nk = 0; nk < maxiter; nk++)
		{
			...
			if(nk == 1 || nk%20 == 0) {
				//cout << "Time_insac: solve(): Time step " << nt << ": Iteration " << nk << ": relative L2 norm of residual = " << resnorm/resnorm0 << ", net mass flux = " << massflux << endl;
				//cout << "  L2 norm of residual = " << resnorm << endl;
			}
			if(resnorm/resnorm0 < tol && fabs(massflux) < sqrt(tol))
			{
				cout << "Time_insac: solve(): Time step " << nt << ": Converged in " << nk << " iterations. Norm of final residual = " << resnorm << ", final net mass flux = " << massflux << endl;
				break;
			}
			...
			for(i = 1; i <= m->gimx()-1; i++)
				for(j = 1; j <= m->gjmx()-1; j++)
				{
					...
					for(k = 1; k < nvar; k++)
						u[k](i,j) = u[k].get(i,j) - res[k].get(i,j) / ( rho*m->gvol(i,j) * (a/gdt + 1.0/dt(i,j)) );
				}
	...
	cout << "Time_insac: solve(): Completed. Final time = " << telapsed << endl;
}

Array2d<double>* Time_insac::getVariables()
{
	return u;
}

Array2d<double>* Time_insac::getResiduals()
{
	return res;
}
